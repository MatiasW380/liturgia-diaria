...
export default async function handler(req, res) {
  ...
  const length = parseInt(req.query.length || '15000', 10);
  ...
  try {
    ...
    if (find) {
      ...
      const desde = Math.max(0, idx - 1500);
      res.status(200).send(html.slice(desde, desde + 6000));
      return;
    }

    res.status(200).send(html.slice(offset, offset + length));
  } catch (error) {
