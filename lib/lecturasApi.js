...
/**
 * Trae SOLO la celebración del día y el Salmo Responsorial desde ACI Prensa
 * (fuente anterior) — Vatican News no los tiene. No toca el resto.
 */
async function obtenerCelebracionYSalmo() {
  try {
    ...
    return { celebracion, salmo, citaSalmo };
  } catch (error) {
    console.error('⚠️ No se pudo obtener celebración/salmo de ACI Prensa:', error.message);
    return { celebracion: '', salmo: '', citaSalmo: '' };
  }
}
...
export async function obtenerLecturasDelDia() {
  try {
    ...
    const { celebracion, salmo, citaSalmo } = await obtenerCelebracionYSalmo();
    ...
    return {
      ...
      salmo,
      citaSalmo,
      fuente: 'Vatican News (Evangelio/Lecturas) + ACI Prensa (Celebración/Salmo)',
    };
  } catch (error) {
